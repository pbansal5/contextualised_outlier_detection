...
class OurModel(nn.Module):
    def __init__(self,sizes,embedding_size=16,ninp=64,nlayers=4,nhid=32,nhead=2,kernel_size=9):
        ...
        num_feats = embedding_size*len(sizes)+1
        self.transformer = TransformerConvModel(num_feats=num_feats,nlayers=nlayers,nhid=nhid,ninp=ninp,kernel_size=kernel_size,nhead=nhead)
        self.outlier_layer1 = nn.Linear(3*len(sizes)+ninp,hidden_dim)
    ...
    def core(self,series,context_info):
      ...
      #  series[np.arange(series.shape[0]),context_info['time']] = series.mean(dim=1)
        ...
        hidden_state = self.transformer(series.transpose(1,2),context_info['time'])
        ...
        feats = torch.cat([temp,hidden_state],axis=1)
