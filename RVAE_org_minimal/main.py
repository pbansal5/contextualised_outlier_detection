...
def main():
    ...
    for epoch in range(1,50 + 1):
        ...
        torch.save(model.state_dict(), os.path.join('/mnt/blossom/more/pbansal/rvae_nyc_org_checkpoints','checkpoint_%d'%epoch))
        loss_ = 0
...
def test_ ():
    ...
    for epoch in range(1,51):
        model.load_state_dict(torch.load(os.path.join('/mnt/blossom/more/pbansal/rvae_nyc_org_checkpoints','checkpoint_%d'%epoch)))
        loss_ = 0
        for (x,_) in val_loader :
            with torch.no_grad():
                p_params,q_params,q_samples = model(x.cuda())
