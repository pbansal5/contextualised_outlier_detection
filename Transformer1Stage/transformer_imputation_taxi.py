...
log_file = 'transformer_taxi_1stage_zero_included_consistent'
...
train_set = TransformerDataset('../dataset/2dnyc_zero_taxi_complete.npy','../dataset/2dnyc_zero_taxi_train_examples.npy')
val_set = TransformerDataset('../dataset/2dnyc_zero_taxi_complete.npy','../dataset/2dnyc_zero_taxi_test_examples.npy')
...
model = OurModel(sizes=[266,266],ninp=128,embedding_size=32,nhid=64,nlayers=4,nhead=4).to(device)
...
for epoch in range(start_epoch,max_epoch):
    ...
    if (epoch == 40):
        torch.save(best_state_dict,'best_taxi_40epochs')
    if (epoch == 100):
        torch.save(best_state_dict,'best_taxi_100epochs')
    if (epoch % 1 == 0):
