...
class OurModel(nn.Module):
    def __init__(self,sizes,embedding_size=16,ninp=64,nlayers=4,nhid=32,nhead=2,kernel_size=9,other_residuals=False):
        ...
        self.transformer = TransformerConvModel(num_feats=embedding_size*len(sizes)+76+28,nlayers=nlayers,nhid=nhid,ninp=ninp,kernel_size=kernel_size,nhead=nhead)
        if (other_residuals):
    ...
    def core(self,series,context_info):
        output = series[np.arange(series.shape[0]),context_info['time'],context_info['index'][:,0]]
        if (self.stage2 == False):
            series[np.arange(series.shape[0]),context_info['time'],context_info['index'][:,0]] = 0
            series[np.arange(series.shape[0]),context_info['time'],context_info['index'][:,1]+self.sizes[0]] = 0
    ...
    def second_moment_forward (self,series,context_info):
        output,mean,std = self.core(series,context_info)
        return {'mae':self.mae_loss(mean,output,context_info['std']),'nll':self.second_moment_nll_loss(mean,std,output)}
    ...
    def second_moment_validate(self,series,context_info):
        ...
        return {'mae':self.mae_loss(mean,output,context_info['std']),'nll':self.second_moment_nll_loss(mean,std,output),'sum':self.sum_(output,context_info['mean'],context_info['std']),
                'crps':self.second_moment_crps_loss(output,mean,std,context_info['mean'],context_info['std'])}
    ...
    def second_moment_nll_loss(self,y_pred,sigma_pred,y):
        err1 = (((y-y_pred)**2/self.softplus(sigma_pred)) + torch.log(self.softplus(sigma_pred)))
        return err1.mean()
    ...
    def second_moment_crps_loss(self,x,mu_pred,sigma_pred,mean,std):
        ...
        x = x.cpu()*std + mean
        sigma_pred = torch.sqrt(self.softplus(sigma_pred)).cpu()*std
...
class TransformerDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self,index):
        ...
        temp = series[time_,this_example[1],this_example[2]]
        series = np.concatenate([series[:,:,this_example[2]],series[:,this_example[1],:]],axis=1)
        
        series[time_,this_example[1]] = 0
        series[time_,this_example[2]+self.feats.shape[1]] = 0
        
        mean = np.nanmean(series,axis=0)
        std = np.maximum(np.nanstd(series,axis=0),1e-7)
        series[time_,this_example[1]] = temp
        series[time_,this_example[2]+self.feats.shape[1]] = temp
        ...
        #hardcoded for now
        if (self.num_dim == 3):
            return torch.FloatTensor(series),time_,[this_example[1],this_example[2]],mean[this_example[1]],std[this_example[1]]
