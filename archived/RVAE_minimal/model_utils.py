...
def get_pi_exact_vec(model, data, p_params, q_params, logit_ret=True):
    ...
    prior_sig = torch.tensor(0.95).type(q_params['z']['mu'].data.type())
    pi_vector = []
    start = 0
    cursor_num_feat = 0

    for feat_select, (_, col_type, feat_size) in enumerate(model.feat_info):
        ...
        if col_type == 'categ':
            ...
            pi_vector.append(torch.clamp(pi_feat,1,1))

            start += feat_size

        elif col_type == 'real':

            # if (feat_select != len(model.feat_info)-1):
            #     continue
            # else : 
